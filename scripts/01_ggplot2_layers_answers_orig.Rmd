...
Here, we use red points as well as open triangles to highlight two-seater cars.
...
It displays the confidence interval around the smooth line.
...
`nrow` controls the number of rows and `ncol` controls the number of columns the panels should be arranged in. `facet_grid()` does not have these arguments because the number of rows and columns are determined by the number of levels of the two categorical variables. `facet_wrap` also has a `dir` argument which controls whether the panels should be arranged horizontally or vertically.
:::
...
```{r fig.width = 5}
# The `width` and `height` parameters control the amount of horizontal and vertical displacement, respectively. 
...
in Figure \@ref(fig:visualization-grammar). You'd then select a coordinate
...
```{r visualization-grammar, echo = FALSE, fig.cap = 'The layered grammar of graphics showing how data is transformed into a plot through statistical transformation, geometric objects, aesthetic mappings, and coordinate systems.'}
