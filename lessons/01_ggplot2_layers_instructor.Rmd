...
- `position = "stack"` (default): Shows total counts with composition breakdown
- `position = "fill"`: Shows proportions (all bars reach 100%)
- `position = "dodge"`: Shows side-by-side for easy comparison
- `position = "jitter"`: Adds random noise (you learned this with `geom_jitter()` earlier!)
...
`position = "stack"` shows actual counts - bars have different heights based on total observations in each category.
