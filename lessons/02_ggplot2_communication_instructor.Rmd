# Communication chapter: Polish for PUBLICATION
Total points: 42
...
The easiest place to start when turning an exploratory graphic into a publication-ready graphic is with good labels. You add labels with the `labs()` function:
...
1. Create a boxplot on the fuel economy data with x = `class` and y = `hwy` with customized `title`, `subtitle`, `caption`, `x`, `y`, and `fill` labels. Make the title describe an interesting pattern you see in the data. +8pts
...
However, the annotated plot we made above is hard to read because the labels overlap with points. We can use the `geom_label_repel()` function from the **ggrepel** package to fix this. This useful package will automatically adjust labels so that they don't overlap:
...
Labels are placed in all facets by default. To add a label to a single facet, add a column to the label data frame with the value of the faceting variable. To put different labels in each facet, include the faceting variable with different values for each label.
...
When you have a predefined mapping between values and colors, use `scale_color_manual()`. For example, if we map presidential party to color, we want to use the standard mapping of red for Republicans and blue for Democrats. One approach is to use `scale_color_manual()`:
...
For continuous color, you can use the built-in `scale_color_gradient()` or `scale_fill_gradient()`. If you have a diverging scale, you can use `scale_color_gradient2()`. That allows you to give a different color for the midpoint (usually 0).

Another option is `scale_color_viridis_c()` provided by the **viridis** package (bundled with **ggplot2**). It's a continuous analog of the categorical ColorBrewer scales. The viridis scales are designed to be perceptually uniform in both color and when converted to black and white, and to be perceptible to people with various forms of color blindness.
...
**Ans-1.4.1.1:**
```{r Ans-1.4.1.1}
# It does not override the default scale because the colors for the default point shape (19, "filled circle") are set by the `color` aesthetic, not the `fill` aesthetic.
...
**Ans-1.4.1.3:**
```{r Ans-1.4.1.3}
# The problem with the legend is that the alpha value makes the colors hard to see. 
# So we'll override the alpha value to make the points solid in the legend.
...
Many more themes are included in add-on packages like **ggthemes** (<https://jrnold.github.io/ggthemes>).

It's also possible to control individual components of each theme, like the size and color of the font used for the axes. We've already seen that `legend.position` controls where the legend is drawn.

For example, in the plot below we change several theme elements:
...
Note that customization of theme elements is done with `element_*()` functions:
- `element_text()` for text elements (size, color, face)
...
1. Pick a theme offered by `ggplot2` or the `ggthemes` package and apply it to this plot: +2pts
...
2. Make the axis labels of your plot blue and bolded. +2pts
...
# This plots p1 in the left half, and p3/p2 stacked in the right half.
# This happens because `/` takes precedence over `+`, like in mathematics: 
# 3 + 4/5 vs. (3 + 4)/5
```
:::
